...
class VideoMetaRecurrentModel(VideoBaseModel):

    def __init__(self, opt):
        super(VideoMetaRecurrentModel, self).__init__(opt)
        if self.is_train:
    ...
    def feed_data(self, data):
        ...
        self.frm_type = data['frm_type'].to(self.device)
        self.IBP_rep_list = data['IBP_rep_list'].to(self.device)
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output = self.net_g(self.lq, self.mv, self.frm_type, self.IBP_rep_list, 'train')
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for i in range(rank, num_folders + num_pad, world_size):
            ...
            val_data['mv'].unsqueeze_(0)
            val_data['frm_type'].unsqueeze_(0)
            val_data['IBP_rep_list'].unsqueeze_(0)
    ...
    def test(self):
        ...
        with torch.no_grad():
            self.output = self.net_g(self.lq, self.mv, self.frm_type, self.IBP_rep_list, 'evel')
