...
class Vimeo90KMetaDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(Vimeo90KMetaDataset, self).__init__()
        self.opt = opt
...
@DATASET_REGISTRY.register()
class Vimeo90KMetaRecurrentDataset(Vimeo90KMetaDataset):

    def __init__(self, opt):
        super(Vimeo90KMetaRecurrentDataset, self).__init__(opt)
    ...
    def __getitem__(self, index):
        ...
        for neighbor in self.neighbor_list:
            if self.is_lmdb:
                img_lq_path = f'{number}/{clip}/{seq}/im{neighbor}'
                img_gt_path = f'{number}/{clip}/{seq}/im{neighbor}'
            else:
                img_lq_path = self.lq_root / self.lq_subroot[sub_id] / number/ clip / seq / f'im{neighbor}.png'
                img_gt_path = self.gt_root / number/ clip / seq / f'im{neighbor}.png'
            # LQ
            img_bytes = self.file_client.get(img_lq_path, 'lq')
            img_lq = imfrombytes(img_bytes, float32=True)
            # GT
        ...
        # get metadata
        meta_path = self.meta_root / str(self.meta_subroot[sub_id] )/ number/ clip / f'{seq}.npz'
        ...
        # print(type(img_mvs[0]), img_mvs[0].shape)
        img_mvs = torch.stack([torch.from_numpy(mv) for mv in img_mvs], dim=0)
        # img_mvs = img2tensor(img_mvs)
        # img_mvs = torch.stack(img_mvs, dim=0)
        ...
        return {'lq': img_lqs, 'gt': img_gts, 'key': key,
                "mv": img_mvs.float(), 'frm_type':frm_types, 'crf_rate':self.lq_subroot[sub_id], 'IBP_rep_list':IBP_rep_list}
