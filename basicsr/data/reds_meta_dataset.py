...
class REDSMateDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(REDSMateDataset, self).__init__()
        self.opt = opt
...
@DATASET_REGISTRY.register()
class REDSMetaRecurrentDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(REDSMetaRecurrentDataset, self).__init__()
        self.opt = opt
        self.gt_root, self.lq_root = Path(opt['dataroot_gt']), Path(opt['dataroot_lq'])
    ...
    def __getitem__(self, index):
        ...
        # get metadata
        meta_path = self.meta_root / str(self.meta_subroot[sub_id] )/ f'{clip_name}.npz'
        frm_type, mv_f, mv_b = get_meta(meta_path,start_frame_idx, end_frame_idx)
        t = len(frm_type)
        ...
        for i in range(t):
            if frm_type[i]=="B":
                ...
                one_hot = torch.tensor([0,0,1])
                if i+1<t:
                    mv_f[i] = -1 * mv_b[i+1]
            else:
                one_hot = torch.tensor([1,0,0])
...
def get_meta(meta_pth, start_idx, end_idx):
    ...
    for mv in mvs:
        ...
        framenum -= 1
        if framenum<start_idx:
            continue
        elif framenum>end_idx:
            break
        framenum = framenum-start_idx
